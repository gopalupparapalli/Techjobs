...
import { Container, Box, Typography, Button, List, ListItem, ListItemText, Divider, Grid,  ListItemIcon } from '@mui/material';
import { LocationOn, Business, Work, Description, Group, School, MonetizationOn, Send, FiberManualRecord } from '@mui/icons-material';
...
<>    <Helmet>
    ...
    <Container style={styles.root}>
      <Box style={styles.header}>
        <Typography variant="h4" component="h1">TECHJOB IS HIRING</Typography>
      </Box>
      <Box style={styles.header}>
        <Typography variant="h4" component="h1">{jobDetails.title}</Typography>
      ...
      <Box style={styles.section}>
        <Typography style={{fontSize:'26px',fontWeight:700}} component="h2">
          <Business style={{ ...styles.icon, color: sectionColors[2] }} /> {jobDetails.company}
        </Typography>
      </Box>
      {/* <Box style={styles.section}>
      ...
      <Box style={styles.section}>
        <Typography style={{fontSize:'26px',fontWeight:700}} component="h2">
          <Description style={{ ...styles.icon, color: sectionColors[4] }} /> Job Description
        </Typography>
        <List>
          {jobDetails.description.map((point, index) => (
            <ListItem key={index} alignItems="flex-start">
              <ListItemIcon style={styles.listItemIcon}>
                <FiberManualRecord style={{ ...styles.circleIcon, color: sectionColors[0] }} />
              </ListItemIcon>
      ...
      <Box style={styles.section}>
        <Typography style={{fontSize:'26px',fontWeight:700}} component="h2">
          <Group style={{ ...styles.icon, color: sectionColors[1] }} /> Age Range
        </Typography>
        <Typography variant="body1" style={{ marginLeft: '25px' }}>
          <FiberManualRecord style={{ ...styles.circleIcon, color: sectionColors[0] }} /> {jobDetails.ageRange}
        </Typography>
      </Box>
      <Box style={styles.section}>
        <Typography style={{fontSize:'26px',fontWeight:700}} component="h2">
          <Work style={{ ...styles.icon, color: sectionColors[2] }} /> Experience
        </Typography>
        <Typography variant="body1" style={{ marginLeft: '25px' }}>
          <FiberManualRecord style={{ ...styles.circleIcon, color: sectionColors[0] }} /> {jobDetails.experience}
        </Typography>
      </Box>
      <Box style={styles.section}>
        <Typography style={{fontSize:'26px',fontWeight:700}} component="h2">
          <School style={{ ...styles.icon, color: sectionColors[3] }} /> Skills
        </Typography>
        <List>
      ...
      <Box style={styles.section}>
        <Typography style={{fontSize:'26px',fontWeight:700}} component="h2">
          <MonetizationOn style={{ ...styles.icon, color: sectionColors[0] }} /> Education
        </Typography>
        <Typography variant="body1" style={{ marginLeft: '25px' }}>
          <FiberManualRecord style={{ ...styles.circleIcon, color: sectionColors[0] }} /> {jobDetails.education}
        </Typography>
      </Box>
      <Divider />
      <Grid container justifyContent="center">
        <Button style={styles.applyButton} variant="contained" endIcon={<Send />} onClick={handleClickOpen}>
          Apply Now
        </Button>
      </Grid>
